...
def test_profile_update(test_client, test_user):
    ...
    '''
    response = test_client.put('/api/user', data=test_user, headers=headers)
    assert response.status_code == 200
    assert response.json['message'] == 'Profile updated'
    response = test_client.get('/api/user', headers=headers)
    assert response.status_code == 200
    assert response.json['email'] == test_user['email']
    assert 'name' in response.json['profile']
...
def test_deactivate(test_client, test_user):
    ...
    '''
    response = test_client.delete('/api/user', headers=headers)
    assert response.status_code == 200
    ...
    response = test_client.put('/api/user', data=test_user, headers=headers)
    assert response.status_code == 401
    response = test_client.post('/api/user', data=test_user, headers=headers)
    assert response.status_code == 401
